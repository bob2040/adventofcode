...
def points(xy_all_int):
    ...
    for i in xy_all_int:
        ...
        # diagonal
            ...
            # from (x,y) to (x-1,y-1),top-left direction
            if i[0] > i[2] and i[1] > i[3]:
                for k in range(abs(i[0]-i[2])+1):
                    a.append(str([i[0]-k, i[1]-k]))
            # from (x,y) to (x-1,y+1),bottom-left direction
            elif i[0] > i[2] and i[1] < i[3]:
                for k in range(abs(i[0]-i[2])+1):
                    a.append(str([i[0]-k, i[1]+k]))
            # from (x,y) to (x+1,y+1),bottom-right direction
            elif i[0] < i[2] and i[1] < i[3]:
                for k in range(abs(i[0]-i[2])+1):
                    a.append(str([i[0]+k, i[1]+k]))
            # from (x,y) to (x+1,y-1),top-right direction
            elif i[0] < i[2] and i[1] > i[3]:
                for k in range(abs(i[0]-i[2])+1):
                    a.append(str([i[0]+k, i[1]-k]))
    b = set(a)
...
if __name__ == '__main__':
    ...
    print(points(xy_all_int))  # 17741
    end = time.time()
    print(end - start)  # 814seconds
